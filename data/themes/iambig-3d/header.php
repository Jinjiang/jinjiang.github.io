...
<form id="search" method="post" action="/">
    ...
    <button><?php _e('搜索'); ?></button>
</form>

<div id="nav">
    <a href="<?php $this->options->siteUrl(); ?>" class="button-3d<?php if($this->is('index')): ?> current<?php endif; ?>"><?php _e('首页'); ?></a>
    <?php $this->widget('Widget_Contents_Page_List')->to($pages); ?>
    <?php while($pages->next()): ?>
    <a href="<?php $pages->permalink(); ?>" title="<?php $pages->title(); ?>" class="button-3d<?php if($this->is('page', $pages->slug)): ?> current<?php endif; ?>"><?php $pages->title(); ?></a>
    <?php endwhile; ?>
</div>
