...
function parse(presentation) {
    ...
    $.each(slides, function (i, slide) {
        ...
        var titleHtml = '';
        var contentHtml = '';
        var linkedContentHtml = '';

        if (slide.values.content) {
            if (slide.values.title == '## HTML5 幻灯片演示') {
                console.log(slide.values.content);
            }
            // linkedContentHtml = slide.values.content.replace(/(^|\n)([^ ].*[^a-z]|)([a-z]+:\/\/[^\s^<^\)^\]]*)/ig, '$1$2[$3]($3)');
            linkedContentHtml = slide.values.content;
            if (slide.values.title == '## HTML5 幻灯片演示') {
                console.log(linkedContentHtml);
            }
        }

        titleHtml = markdown.toHTML(slide.values.title || '');
        contentHtml = markdown.toHTML(linkedContentHtml);
